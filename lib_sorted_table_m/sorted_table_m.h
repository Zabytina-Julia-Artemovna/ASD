...
private:
    ...
    std::pair<size_t, bool> find_position(const TKey& key) const {
        size_t left = 0;
...
template <class TKey, class TValue>
SortedTableM<TKey, TValue>& SortedTableM<TKey, TValue>::operator=(const SortedTableM<TKey, TValue>& other) {
    if (this != &other) {
        _rows = other._rows;
    }
    return *this;
...
template <class TKey, class TValue>
TValue& SortedTableM<TKey, TValue>::find(const TKey& key) {
...
template <class TKey, class TValue>
size_t SortedTableM<TKey, TValue>::size() const noexcept {
