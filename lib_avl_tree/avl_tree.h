...
void AVLTree<TKey, TValue>::erase_recursive(
    ...
    if (key < node->data_.first) {
        ...
        if (!node->left_ || !node->right_) {
            // Случай 1 или 2: 0 или 1 ребёнок
            ...
            delete node;
            node = child;
            //  НЕ ВОЗВРАЩАЕМСЯ — нужно продолжить балансировку
        }
        else {
            // Случай 3: 2 ребёнка
            ...
            // Рекурсивно удаляем заменяющий узел
    ...
    // Балансировка выполняется для всех узлов на пути к корню
    if (node) {
        recalc_height(node);
