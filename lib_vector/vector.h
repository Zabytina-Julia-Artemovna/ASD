...
public:
    ...
    inline const T* get_data() const noexcept { return _data; }
    inline T* get_data() noexcept { return _data; }
    inline const State* get_states() const noexcept { return _states; }
    inline State* get_states() noexcept { return _states; }
    ...
    size_t get_deleted() const noexcept { return _deleted; }
    size_t get_capacity() const noexcept { return _capacity; }

    inline Iterator begin() const noexcept { return Iterator(_data); }
    inline Iterator end() const noexcept { return Iterator(_data + _size); }
...
Tvector<T>::Tvector() noexcept
    : _size(0), _capacity(0), _data(nullptr), _states(nullptr), _deleted(0) {}
...
Tvector<T>::Tvector(const Tvector<T>& other_vector) {
    ...
    for (size_t i = 0; i < _capacity; ++i) {
        if (other_vector._states[i] == State::busy) {
...
void Tvector<T>::pop_back() {
    if (_size == 0) throw std::out_of_range("Vector has size = 0");
...
void Tvector<T>::erase(size_t position) {
    if (position >= _size) throw std::out_of_range("Invalid position");

    size_t real_pos = get_real_position(position);

    if (_states[real_pos] == State::busy) {
...
size_t find_first_element(const Tvector<U>& object, const U& value) {
    ...
    for (size_t i = 0; i < object._size; i++) {
        if (object._states[i] == Tvector<U>::State::deleted) {
            continue;
        }
        else if (object._data[i] == value && object._states[i] == Tvector<U>::State::busy) {
            return result + 1;
        }
        result++;
    }
...
size_t find_last_element(const Tvector<U>& object, const U& value) {
    ...
    for (size_t i = 0; i < object._size; i++) {
        ...
        if (object._data[i] == value && object._states[i] == Tvector<U>::State::busy) {
